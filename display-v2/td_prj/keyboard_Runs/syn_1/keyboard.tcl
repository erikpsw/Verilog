...
set rc [catch {
  open_project {keyboard.prj}
  import_device eagle_s20.db -package EG4S20BG256
  elaborate -top {keyboard_instance}
  export_db {keyboard_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../../dc/keyboard.adc
  optimize_rtl
  report_area -file keyboard_rtl.area
  export_db {keyboard_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  read_sdc
  optimize_gate -maparea keyboard_gate.area
  legalize_phy_inst
  export_db {keyboard_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
