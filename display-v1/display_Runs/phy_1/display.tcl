...
set rc [catch {
  ...
  import_db {../syn_1/display_gate.db}
  read_sdc
  place
  export_db {display_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file display_phy.area
  export_db {display_pr.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid display_inst.bid
  bitgen -bit "display.bit" -unused_io_status pulldown
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
